...
class TestApi(unittest.TestCase):
    ...
    def test_ner_endpoint_given_json_body_returns_200(self):
        with app.test_client() as client:
            response = client.post('/ner', json={"sentence":"Theo Katzman is an amazing guitarist."})
            assert response._status_code == 200

    def test_ner_endpoint_given_json_body_with_known_entities_returns_rentity_result_in_response(self):
        with app.test_client() as client:
